...
|`Last-Modified`|The date and time that the blob was last modified. The date format follows RFC 1123. For more information, see [Representation of Date-Time Values in Headers](../rest-conceptual/Representation-of-Date-Time-Values-in-Headers.md).<br /><br /> Any write operation on the blob, including updates to the blob’s metadata or properties, changes the last modified time of the blob.|  
...
## Remarks  
 ...
 To create a new page blob, call [Put Blob](../rest-conceptual/Put-Blob.md) and specify the type of blob to create as a page blob. A page blob may be up to 1 TB in size.  
...
-   You can call [Lease Blob](../rest-conceptual/Lease-Blob.md) to lock the blob against other writes for a one-minute period, or longer if the lease is renewed.  
...
1.  The client creates a page blob with [Put Blob](../rest-conceptual/Put-Blob.md) and sets its sequence number to 0.  
