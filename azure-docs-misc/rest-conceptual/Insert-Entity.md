...
|`Prefer`|Optional. Specifies whether the response should include the inserted entity in the payload. Possible values are `return-no-content` and `return-content`. For more information, see [Setting the Prefer Header to Manage Response Echo on Insert Operations](../rest-conceptual/Setting-the-Prefer-Header-to-Manage-Response-Echo-on-Insert-Operations.md).|  
...
### Request Body  
 The `Insert Entity` operation sends the entity to be inserted as an OData entity, which is either a JSON or an Atom feed. For more information, see [Inserting and Updating Entities](../rest-conceptual/Inserting-and-Updating-Entities.md).  
...
## Remarks  
 ...
 For information about performing batch insert operations, see [Performing Entity Group Transactions](../rest-conceptual/Performing-Entity-Group-Transactions.md).  
