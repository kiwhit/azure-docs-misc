...
-   Specify the HTTP protocol from which to accept requests (either HTTPS or HTTP/HTTPS).  
 ...
 A service-level SAS, by contrast, delegates access to a resource in just one of the storage services: the Blob, Queue, Table, or File service. For more information on service SAS, see [Constructing a Service SAS](../rest-conceptual/Constructing-a-Service-SAS.md).  
