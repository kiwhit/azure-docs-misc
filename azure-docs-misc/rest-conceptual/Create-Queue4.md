...
## Remarks  
 ...
 For guidance about valid queue names, see [Naming Queues and Metadata](../rest-conceptual/Naming-Queues-and-Metadata.md). If the specified queue name is not a valid name, the `Create Queue` operation returns status code 400 (Bad Request), along with additional error information, as shown in the following example.  
