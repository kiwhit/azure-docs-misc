...
|`Shared Read/Write/Delete`<br /><br /> `(Deny Nothing)`|Delete File|  
 ...
 The File service will return sharing violations only when files are open on SMB clients. Note that for a File service [Delete File](../rest-conceptual/Delete-File2.md) operation to succeed, there can be no SMB clients with handles open against that file. Please refer to the [Delete File](../rest-conceptual/Delete-File2.md) operation and the section below titled **Interaction between the File Service and SMB Opportunistic Locks** for more details.  
