...
-   Specifying the `$format` URI parameter overrides the value specified in the `Accept` request header, when the OData data service version is set to 3.0. See [Setting the OData Data Service Version Headers](../rest-conceptual/Setting-the-OData-Data-Service-Version-Headers.md) for details about the OData service version.  
...
### Examples:  
 The following sample OData entry demonstrates the JSON format sent as a request to insert an entity into Azure Table storage (see [Insert Entity](../rest-conceptual/Insert-Entity.md) for details on the insert operation):  
...
```  
 ...
 When a client queries a set of entities in Azure Table storage, the service responds with a JSON payload (see [Query Entities](../rest-conceptual/Query-Entities.md) for details on the query operation). The feed can include one of the three levels of information: no metadata, minimal metadata, or full metadata. The following examples demonstrate each kind:  
...
## Atom Format (application/atom+xml)  (Versions earlier than 2015-12-11 only)  
 ...
 For example, the following sample OData entry demonstrates the Atom format sent through a request to insert an entity into Azure Table storage using the REST API (see [Insert Entity](../rest-conceptual/Insert-Entity.md) for details on the insert operation):  
...
```  
 ...
 When a client queries a set of entities in Table storage, the service responds with an Atom feed, which is a collection of Atom entries (see [Query Entities](../rest-conceptual/Query-Entities.md) for details on the query operation):  
...
### Property Types in an Atom Feed  
 ...
 If a property is created with an explicit type, a query that returns the entity includes that type within the Atom feed, so that you can determine the type of an existing property if necessary. Knowing a property's type is important when you are constructing a query that filters on that property. For more information, see [Querying Tables and Entities](../rest-conceptual/Querying-Tables-and-Entities.md).  
