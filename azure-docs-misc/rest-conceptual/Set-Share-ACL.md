...
## Remarks  
 ...
 When you set permissions for a container, the existing permissions are replaced. To update the container's permissions, call [Get Share ACL](../rest-conceptual/Get-Share-ACL.md) to fetch all access policies associated with the container, modify the access policy that you wish to change, and then call **Set Share ACL** with the complete set of data to perform the update.  
