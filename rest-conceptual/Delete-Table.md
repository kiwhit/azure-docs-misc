...
### Request Headers  
 The request must include the `Authorization` header and either the `Date` or `x-ms-date` header, as specified in [Authentication for the Azure Storage Services](../rest-conceptual/Authentication-for-the-Azure-Storage-Services.md). Additional headers are described in the following table.  
