...
|`ContainerSas`|String|Required for import jobs if and only if `StorageAccountKey` is not specified. The container SAS for accessing the blobs associated with the job. See [Put Job](../StorageImportExportREST/Put-Job.md) for its format.This element is omitted from the manifest for an export operation.|  
