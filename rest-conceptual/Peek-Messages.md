...
```  
 ...
 Note that this response differs from the response for the [Get Messages](../rest-conceptual/Get-Messages.md) operation in that the response does not contain the `PopReceipt` or `TimeNextVisible` elements.  
...
## Remarks  
 When a message is retrieved from the queue with `peekonly` set to `true`, the visibility of the message is unchanged. The message remains available to other consumers of the queue until a client retrieves the message with the [Get Messages](../rest-conceptual/Get-Messages.md) operation. The client requesting the message does not receive a pop receipt with the response and therefore cannot delete the message.  
