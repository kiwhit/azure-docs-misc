...
### Request Body  
 The `Update Entity` operation sends the entity to be updated as an OData entity set, which may be either a JSON or an Atom feed. For more information, see [Inserting and Updating Entities](../StorageServicesREST/Inserting-and-Updating-Entities.md).  
...
## Remarks  
 ...
 If the `If-Match` header is missing from the request in versions 2011-08-18 and later, then the service performs an [Insert Or Replace Entity](../StorageServicesREST/Insert-Or-Replace-Entity.md) (upsert) operation. In versions prior to 2011-08-18, the service returns status code 400 (Bad Request).  
...
>  You can take advantage of this behavior to remove a property from an entity.  
 ...
 For information about performing batch update operations, see [Performing Entity Group Transactions](../StorageServicesREST/Performing-Entity-Group-Transactions.md).  
