...
|`<operation-type>`|string|The type of REST operation performed. See the [Storage Analytics Logged Operations and Status Messages](../StorageServicesREST/Storage-Analytics-Logged-Operations-and-Status-Messages.md) topic for a list of possible operations.|`GetBlob`|  
|`<request-status>`|string|The status of the requested operation. See the [Storage Analytics Logged Operations and Status Messages](../StorageServicesREST/Storage-Analytics-Logged-Operations-and-Status-Messages.md) topic for a list of possible status messages.|`Success`|  
...
|`<requester-account-name>`|string|The account name making the request, if the request is authenticated. This field will be empty for anonymous and SAS requests.|`myaccount`|  
