...
## Remarks  
 ...
 To construct a request for a read operation against the secondary endpoint, append `-secondary` as a suffix to the account name in the URI that you use to read from Blob storage. For example, a secondary URI for the [Get Blob](../StorageServicesREST/Get-Blob.md) operation will be similar to `https://myaccount-secondary.blob.core.windows.net/mycontainer/myblob`.  
