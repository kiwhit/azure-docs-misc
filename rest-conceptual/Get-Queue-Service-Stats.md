...
## Remarks  
 ...
 To construct a request for a read operation against the secondary endpoint, append `-secondary` as a suffix to the account name in the URI that you use to read from Queue storage. For example, a secondary URI for the [Peek Messages](../rest-conceptual/Peek-Messages.md) operation will be similar to `https://myaccount-secondary.queue.core.windows.net/myqueue/messages?peekonly=true`.  
